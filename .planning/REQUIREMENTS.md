...
- [~] **HOST-07**: Strict CSP rolled to enforced + `style-src 'self'` (no `'unsafe-inline'`) after Phase 4 inline-style sweep is complete (closes H4 fully) — Wave 1 substrate COMPLETE 2026-05-10 via Plan 10-01 (162 static inline-style attrs migrated to utility classes; src/main.js grep `style:\s*"` returns 0); Wave 2 substrate COMPLETE 2026-05-10 via Plan 10-02 (firebase.json CSP-RO directive value tightened: style-src `'self'` only / connect-src + `https://de.sentry.io` / frame-src `'self'`; tests/firebase-config.test.js + 6 schema assertions in `firebase.json — Phase 10 tightened CSP shape (HOST-07)` describe block; header KEY remains `Content-Security-Policy-Report-Only` until Wave 4); enforcement flip lands in Plan 10-04
...
| HOST-06, HOST-07                         | Phase 10                               | HOST-07 Waves 1+2 substrate Validated 2026-05-10 (Plans 10-01 + 10-02); HOST-06 substrate (preload-eligibility schema test) Validated 2026-05-10 (Plan 10-02); HOST-07 enforcement Pending (Plan 10-04); HOST-06 hstspreload.org submission Pending (Plan 10-05) | Plan 10-01 closed inline-style sweep (162 sites migrated); Plan 10-02 tightened CSP-RO directive value + 6 schema assertions; Plans 10-03..10-05 pending                                  |
