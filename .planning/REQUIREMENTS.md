...
- [~] **OBS-04**: Source maps uploaded to Sentry via `@sentry/vite-plugin` in CI (release-tagged) — substrate code-complete Phase 9 Wave 2 (`vite.config.js` plugin registered conditionally on `SENTRY_AUTH_TOKEN` + EU region URL `https://de.sentry.io/` + `filesToDeleteAfterUpload: ["dist/**/*.map"]`; `.github/workflows/ci.yml` build/deploy/preview env wiring + post-build .map deletion gate on deploy + preview); operator must set `SENTRY_AUTH_TOKEN` + `VITE_SENTRY_DSN` GitHub Actions secrets per `runbooks/phase-9-sentry-bootstrap.md` Step 5 before first deploy actually uploads — verified at Wave 5 close-gate via first-deploy run logs (Plan 09-02, commits `fc29a4f`, `f3978eb`)
- [~] **OBS-05**: Cloud Function alert (Slack webhook) fires on auth anomalies: >5 failed sign-ins from the same IP in 5min, MFA disenrolment, role escalation, unusual-hour `gdprExportUser` calls — **Server-side audit-event substrate for Rules 1+3 landed Phase 9 Wave 3 (Plan 09-03a):** `auth.signin.failure` substrate wired into `beforeUserSignedIn` catch branch (DORMANT today; populates the moment any future plan adds an explicit rejection rule); `iam.claims.set` server-side bare emission lands on every admin claim mutation. Plan 09-04 will land the `authAnomalyAlert` `onDocumentCreated` trigger that consumes these rows. Rule 2 (MFA disenrolment) deferred until `enrollTotp`/`unenrollAllMfa` deps land in `src/main.js` (currently `// deferred to user-testing phase` per D-09-03a-4).
