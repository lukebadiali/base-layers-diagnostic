...
- [~] **Phase 11: Documentation Pack (Evidence Pack)** â€” `SECURITY.md` + `PRIVACY.md` + `THREAT_MODEL.md` + `docs/CONTROL_MATRIX.md` + `docs/RETENTION.md` + `docs/IR_RUNBOOK.md` + `docs/DATA_FLOW.md` + `/.well-known/security.txt` + `docs/evidence/` VSQ-ready screenshots; every claimed control has code link + test link + framework citation. (Plans 11-01..11-06 autonomous portions COMPLETE 2026-05-10 â€” 6/6 waves landed; 11 net-new test files; 88 CONTROL_MATRIX rows; 22 docs/evidence inventory rows; SECURITY.md Â§ Phase 11 Audit Index + line-number drift sweep; runbooks/phase-11-cleanup-ledger.md zero-out gate. Phase 11 close pending operator /gsd-verify-work 11 per 11-06 Task 5 checkpoint:human-verify gate.)
...
**Plans**: 6 plans
  ...
  - [x] 11-04-PLAN.md — Wave 4: docs/RETENTION.md expansion to 8+ data classes (preserving FN-09 verbatim) + docs/IR_RUNBOOK.md (5 scenarios) + 3 skeleton ir-*.md runbooks (DOC-05 + DOC-06). **COMPLETE 2026-05-10.** docs/RETENTION.md expanded from 1 row (FN-09 rate-limit, 62 lines) to 8 new data-class sections + preserved FN-09 + intro (9 `## ` headings total, 331 lines): Org + User data (soft-delete 30d + permanentlyDeleteSoftDeleted Day-30 sweep + gdprEraseUser Art. 17) / Audit log (12mo hot Firestore + 7y BigQuery archive; Pitfall 11 carve-out) / Firestore export backups (30d Standard → Nearline @ 30d → Archive @ 365d lifecycle) / Cloud Storage object versions (90d post-deletion) / Chat messages + comments (cascade-delete + PII-scrub pre-write) / Documents (cascade + Storage versioning 90d) / Authentication failure counters (per-uid + IP-hashed buckets auto-purged via window-tick) / redactionList entries (7y matching BigQuery archive). 5-axis structure per row: retention period + basis + deletion mechanism + threat coverage + implementation cross-reference table. FN-09 Rate Limiting preserved verbatim. docs/IR_RUNBOOK.md authored (214 lines, 8 `## ` sections): 5 Scenario sections + Comms templates (Internal Slack #ops + Customer-facing GDPR Art. 33 + Status-page v2) + RCA template (Markdown skeleton). 3 net-new skeleton IR runbooks: ir-credential-compromise.md (72 lines) / ir-dependency-cve.md (98 lines) / ir-supply-chain-compromise.md (101 lines) — each a real outline operator can fill on first use mirroring runbooks/phase6-mfa-recovery-drill.md shape. Pitfall 6 cross-reference existence gate (LOAD-BEARING Test 5 of ir-runbook-shape): every cited runbooks/*.md path resolves to a real file via existsSync (7 unique paths cited, all existsSync == true). 15-case TDD doc-shape gate (7 retention + 8 ir-runbook) RED (commit cdb3ed1) → GREEN (commit 1f69004) → 15/15 PASS. Full suite 531 passed + 6 skipped (was 516 + 6; +15; zero regressions across 73 prior test files). Plan-cited gates: `grep -c "^## " docs/RETENTION.md` 9 (≥ 9); `grep -cE "^## Scenario [1-5] " docs/IR_RUNBOOK.md` 5 (== 5); `grep -c "FN-09" docs/RETENTION.md` 1 (preserved); `ls runbooks/ir-*.md | wc -l` 3; Pitfall 19 forbidden-words 0 hits in both new docs. Zero deviations + zero stubs + zero threat flags. Commits: cdb3ed1 (test — RED), 1f69004 (docs — RETENTION + IR_RUNBOOK + 3 skeletons GREEN). Plan 11-05 (security.txt RFC 9116; DOC-08) is unblocked.
  - [x] 11-05-PLAN.md — Wave 5: public/.well-known/security.txt RFC 9116 + firebase.json /.well-known/** Cache-Control 24h entry + 2 build tests (DOC-08). **COMPLETE 2026-05-10.** `public/.well-known/security.txt` authored (410 bytes, 11 lines, 5 RFC 9116 fields — Contact: mailto:security@bedeveloped.com + Expires: 2027-05-10T00:00:00Z + Preferred-Languages: en + Canonical: https://baselayers.bedeveloped.com/.well-known/security.txt + Policy: https://baselayers.bedeveloped.com/SECURITY.md). ASCII-only verified (zero non-ASCII bytes; no BOM); Expires gives 365-day buffer from authoring, 335 days over the 30-day-gate test threshold. `firebase.json` gains new `/.well-known/**` headers entry (Cache-Control: public, max-age=86400 — 24h NOT immutable; plus Content-Type: text/plain; charset=utf-8) appended after the existing 3 entries (all preserved unchanged byte-for-byte). The new entry's max-age=86400 overrides the existing `**/*.@(js|css|png|svg|woff2|ico)` immutable wildcard for files under `/.well-known/` per Firebase Hosting later-entry-wins rule, so yearly Expires rotation propagates within a day (T-11-05-03 Cache-Control-staleness mitigation). Vite default `publicDir` behaviour verified copying file to `dist/.well-known/security.txt` byte-for-byte at build (410 bytes vs 410 bytes; `diff` returns zero); no vite.config.js modifications required. 8-case TDD build-shape gate locks the structural contract — `tests/build/security-txt-fresh.test.js` (5 cases: Contact + Expires-30-day-buffer + Canonical + ASCII-only + Policy field) mirrors 11-RESEARCH.md §"Code Examples" verbatim plus the Policy case added per plan <behavior>; `tests/build/security-txt-served.test.js` (3 cases: publicPath existsSync + dist matches public byte-for-byte + firebase.json /.well-known/** Cache-Control max-age=86400 not immutable) mirrors `tests/build/source-map-gate.test.js` Phase 9 OBS-04 pattern. Pitfall 3 mitigation operational: build fails at CI BEFORE security.txt drifts within 30 days of Expires; fires during routine CI rather than silently after production expiry. RED gate (commit 6f24818) → GREEN gate (commit 29f6bb2): 8/8 PASS post-build; firebase-config baseline 24 passed | 6 skipped (30) preserved; full suite 539 passed + 6 skipped (was 531 + 6 pre-Wave-5; +8 from new build tests; zero regressions across 75 prior test files). 1 auto-fixed deviation Rule 2 (substantive intent preserved via glyph normalisation): D-11-05-01 — security.txt heading comment em-dash (— U+2014; 3-byte UTF-8 sequence with bytes >= 0x80) inherited from IETF reference text replaced with two ASCII hyphens (--) because plan must_have line 18 + Test 4 charCodeAt<128 assertion require ASCII-only; substantive intent preserved; only glyph form normalised per RFC 9116 §2.2 field-value-octets-<0x80 strictness. Zero stubs (Acknowledgments + Encryption OPTIONAL RFC 9116 fields intentionally OMITTED per RESEARCH.md line 551 v2-deferral; substrate-honest scope decision aligned with "credible, not certified" project posture; Wave 6 forward-tracking row F-DOC-08-v2 queued for v2 review). Zero threat flags. All 5 plan-threat-register STRIDE mitigations LANDED (T-11-05-01 Expires drift / T-11-05-02 ASCII drift / T-11-05-03 Cache-Control immutable staleness / T-11-05-04 Vite drops public/.well-known / T-11-05-05 wrong Policy URL). Production deploy of new firebase.json entry is operator-pending — bundled into existing Phase 10 10-DEFERRED-CHECKPOINT.md Step 3 `firebase deploy --only hosting --project bedeveloped-base-layers` (no new operator interrupt; Pitfall 8 selective-deploy boundary preserved). Commits: 6f24818 (test — RED), 29f6bb2 (docs — security.txt + firebase.json GREEN). Plan 11-06 (Wave 6 — DOC-04 CONTROL_MATRIX row population + DOC-09 evidence-pack inventory + DOC-10 final pass + REQUIREMENTS.md DOC-01..09 flips + cleanup-ledger zero-out + /gsd-verify-work 11) is unblocked — DOC-08 substrate at /.well-known/security.txt is the final canonical artefact required before Wave 6.
...
Plans:
- [x] 12-01-PLAN.md — Wave 1: docs/SECURITY_AUDIT_TRANSLATION.md + tests/security-audit-translation-shape.test.js (WALK-01) — COMPLETE 2026-05-11 (commits `32fd813` + `985e878` + `40f939d`)
- [x] 12-02-PLAN.md — Wave 2: SECURITY_AUDIT_REPORT.md Discovery + Executive Summary + locked posture statement + §2 OWASP A01..A10 + §3 + §4 + §5 + tests/security-audit-report-shape.test.js (6 cases; WALK-02/03 substrate) — COMPLETE 2026-05-11 (commits `3fdb4ce` + `33e1f67`)
- [x] 12-03-PLAN.md — Wave 3: SECURITY_AUDIT_REPORT.md §6 OWASP LLM Top 10 2025 (11 enumerated N/A rows — WALK-04 anchor) + §7 + §10 + §13 Sign-off Checklist (46 enumerated rows: 14 Universal + 11 Supabase + 10 Vercel + 11 AI/LLM) + Tests 4 + 8 added to security-audit-report-shape.test.js (8 total) + tests/security-audit-report-paths-exist.test.js (WALK-04 closed) — COMPLETE 2026-05-11 (commits `bb706e0` + `4cc841f` + `cee0b49`)
- [x] 12-04-PLAN.md [autonomous: false] — Wave 4: SECURITY.md § Phase 12 Audit Index (DOC-10 final increment) + REQUIREMENTS.md WALK-01..04 + DOC-10 flips + ROADMAP.md Progress Table flip + runbooks/phase-12-cleanup-ledger.md zero-out + /gsd-verify-work 12 (DOC-10 + WALK-01..04 milestone close) — autonomous portion COMPLETE 2026-05-11 (Tasks 1-4); Task 5 /gsd-verify-work 12 operator gate pending
...
| 11. Documentation Pack (Evidence Pack) | 5/6 | In Progress — Plans 11-01 + 11-02 + 11-03 + 11-04 + 11-05 COMPLETE (Wave 1 DOC-01 + DOC-04 substrate; Wave 2 DOC-02 PRIVACY.md substrate; Wave 3 DOC-03 THREAT_MODEL.md + DOC-07 docs/DATA_FLOW.md substrate; Wave 4 DOC-05 RETENTION.md expansion + DOC-06 IR_RUNBOOK.md + 3 skeleton ir-*.md runbooks; Wave 5 DOC-08 /.well-known/security.txt RFC 9116 + firebase.json /.well-known/** Cache-Control max-age=86400 entry + 2 build tests); Plan 11-06 remaining; Wave 6 ends with /gsd-verify-work 11 operator gate | 2026-05-10 (Plans 11-01..11-05 autonomous portion) |
| 12. Audit Walkthrough + Final Report | 4/4 | Complete (autonomous portion); /gsd-verify-work 12 operator gate pending — Plans 12-01 + 12-02 + 12-03 + 12-04 substrate-COMPLETE (Wave 1 WALK-01 translation map + 4-case schema gate; Wave 2 WALK-02 + WALK-03 substrate — SECURITY_AUDIT_REPORT.md Executive Summary + Discovery + §2 OWASP A01..A10 + §3 + §4 + §5 + 6-case schema gate; Wave 3 WALK-02 + WALK-03 + WALK-04 completion — §6 LLM Top 10 11 N/A rows + §7 16 attack-class rows + §10 10 toolchain rows + §13 Sign-off Checklist 46 enumerated rows + Closing notes; 8-case shape gate + 3-case paths-exist gate; Wave 4 DOC-10 final increment — SECURITY.md § Phase 12 Audit Index + REQUIREMENTS.md WALK-01..04 + DOC-10 flips + runbooks/phase-12-cleanup-ledger.md zero-out). Task 5 /gsd-verify-work 12 operator checkpoint remaining — milestone-close trigger. | 2026-05-11 |
