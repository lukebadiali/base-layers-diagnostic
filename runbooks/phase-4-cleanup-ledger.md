...
| Phase 3 — meta-CSP regression guard                              | Phase 3 (`03-PREFLIGHT.md ## index.html meta-CSP scan`) confirmed `index.html` has no `<meta http-equiv="Content-Security-Policy">` tag (T-3-7 mitigated by absence). Phase 4's modular split touches `index.html`; without a CI guard the tag could be re-introduced silently and conflict with the header CSP shipped in `firebase.json`. T-3-meta-csp-conflict mitigation. | Phase 4 (CODE-01)  | Add a CI test `tests/index-html-meta-csp.test.js` that fails if `index.html` contains `http-equiv="Content-Security-Policy"`. Mirror the schema-test pattern from `tests/firebase-config.test.js` (read file, regex-assert absence, `expect(...).toBe(false)`). Optionally also add a `.husky/pre-commit` grep as a pre-CI gate. Close this row when Phase 4 lands the test. |
