...
Closed in Phase 4 Wave 5 (04-05): **all 12 app.js rows** — `app.js` was DELETED entirely in the terminal cutover commit (D-03). The renamed file lives at `src/main.js` with the IIFE body preserved per Wave 4 Dev #1 + Wave 3 Dev #1 + D-12 + threat-model T-4-5-2 (rendered-DOM drift = unintended UI change). The 11 in-IIFE `eslint-disable-next-line` rows that lived inside `app.js` followed the IIFE body to `src/main.js` — they remain in the IIFE closure but the file path tracker rotates from `app.js:N` to `src/main.js:N` (Wave 6 cleanup closes those when IIFE bodies migrate into views/* stubs). The `@ts-nocheck` row also rotates from `app.js:1` to `src/main.js:1` for the same reason. **The 12 entries below are CLOSED** as their tracker (file path = app.js) no longer exists. Wave 6 follow-up tracking is documented in the Wave 5 → Wave 6 carry-forward section below.
Remaining: **0** in-app.js rows (file deleted); **12 rotated rows** at src/main.js:N tracking the same suppressions for Wave 6 closure when IIFE body migrates to views/*.
...
The 12 rows above are **CLOSED** by their tracker (app.js) being deleted in Wave 5. The underlying suppressions migrated atomically with the IIFE body to `src/main.js` (where they remain at the new file path until Wave 6 cleanup migrates IIFE bodies into views/* stubs). Wave 6 will close each rotated row when the corresponding IIFE-resident render function moves to its `src/views/{name}.js` stub. Wave 6 entry adds these rows to a new "src/main.js IIFE residue (Wave 6 cleanup target)" subsection.
